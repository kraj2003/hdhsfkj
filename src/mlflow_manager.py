# src/mlflow_manager.py - Enhanced Version with Run Diff
import mlflow
import mlflow.keras
...
class MLflowManager:
        ...
        with mlflow.start_run():
            ...
            try:
                ...
                with open("git_diff.patch", "w", encoding="utf-8") as f:
                    f.write(diff_output)
                mlflow.log_artifact("git_diff.patch")
            except Exception as e:
                print(f"Warning: could not log git diff → {e}")
            ...
            # ----------------------- RUN DIFF FEATURE -----------------------
            self._log_run_diff(config, metrics)
    ...
    def _log_precision_recall_curve(self, y_true, y_pred_prob):
        ...
        plt.axvline(best_threshold, color='red', linestyle='--', label=f'Best F1 Threshold: {best_threshold:.3f}')
        ...
        mlflow.log_metric("best_f1_score", f1_scores[best_idx])
    ...
    def _log_training_curves(self, history):
        ...
        plt.title('Model Loss'); plt.xlabel('Epoch'); plt.ylabel('Loss'); plt.legend(); plt.grid(True, alpha=0.3)

        plt.subplot(1,3,2)
        plt.plot(history.history['accuracy'], label='Training Accuracy', color='blue')
        plt.plot(history.history['val_accuracy'], label='Validation Accuracy', color='red')
        plt.title('Model Accuracy'); plt.xlabel('Epoch'); plt.ylabel('Accuracy'); plt.legend(); plt.grid(True, alpha=0.3)
        ...
        if 'Recall' in history.history:
            ...
            plt.title('Model Recall'); plt.xlabel('Epoch'); plt.ylabel('Recall'); plt.legend(); plt.grid(True, alpha=0.3)
    ...
    def _log_probability_analysis(self, y_pred_prob, y_true, y_pred):
        ...
        high_conf_correct = np.sum((prob_flat > high_conf_threshold) & (y_pred==y_true))
        high_conf_total = np.sum(prob_flat > high_conf_threshold)
        low_conf_correct = np.sum((prob_flat < low_conf_threshold) & (y_pred==y_true))
        low_conf_total = np.sum(prob_flat < low_conf_threshold)
        ...
        plt.plot(bin_centers, bin_accs,'ro-', label='Model Calibration')
        plt.xlabel('Mean Predicted Probability'); plt.ylabel('Fraction of Positives'); plt.title('Calibration Plot'); plt.legend(); plt.grid(True, alpha=0.3)
        ...
        plt.xlabel('Confidence Score'); plt.ylabel('Frequency'); plt.title('Model Confidence Distribution'); plt.grid(True, alpha=0.3)
    ...
    def _get_last_run(self):
        ...
        if len(runs) > 0:
            run_data = runs.iloc[0].to_dict()
            last_run = {
                "params": {k.replace("params.",""): v for k,v in run_data.items() if k.startswith("params.")},
                "metrics": {k.replace("metrics.",""): v for k,v in run_data.items() if k.startswith("metrics.")}
            }
            return last_run
        return {"params": {}, "metrics": {}}

    def _log_run_diff(self, current_config, current_metrics):
        last_run = self._get_last_run()
        diffs = {}

        for k,v in current_config.items():
            prev = last_run["params"].get(k)
            if str(prev) != str(v):
                diffs[f"param_{k}"] = {"previous": prev, "current": v}

        for k,v in current_metrics.items():
            prev = last_run["metrics"].get(k)
            if prev is None or prev != v:
                diffs[f"metric_{k}"] = {"previous": prev, "current": v, "improved": None if prev is None else v > prev}

        if diffs:
            with open("run_diff.json","w") as f:
                json.dump(diffs, f, indent=4)
            mlflow.log_artifact("run_diff.json")
            mlflow.log_metric("num_param_changes", sum(1 for key in diffs if key.startswith("param_")))
            mlflow.log_metric("num_metric_changes", sum(1 for key in diffs if key.startswith("metric_")))
